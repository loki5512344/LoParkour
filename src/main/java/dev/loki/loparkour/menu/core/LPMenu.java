...
public abstract class LPMenu {
    ...
    protected String locale(@NotNull Player player) {
        var user = dev.loki.loparkour.player.core.ParkourUser.getUser(player);
    ...
    protected ItemStack backItem(@NotNull Player player) {
        return localeItem(player, "other.back");
    }

    protected ItemStack nextItem(@NotNull Player player) {
        return localeItem(player, "other.next");
    }
