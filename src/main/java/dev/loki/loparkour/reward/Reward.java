package dev.loki.loparkour.reward;

import dev.loki.loparkour.LoParkour;
import dev.loki.loparkour.api.Registry;
import dev.loki.loparkour.hook.VaultHook;
    ...
    public void execute(@NotNull ParkourPlayer player, @NotNull Mode mode) {
        ...
        if (string.toLowerCase().contains("send:")) {
            ...
            try {
                ...
                LoParkour.logging().stack("Error while trying to process Vault reward", "check your rewards file for incorrect numbers", ex);
            }
        } else {
