...
/**
 * Generation-related configuration options (jump types, validation, memory, ghost mode).
 */
public class OptionGeneration {
    ...
    public static void init() {
        ...
        initMemory();
        initGhostMode();
        initGenerationProbabilities();
    }

    private static void initJumpValidation() {
        JUMP_VALIDATION_ENABLED = Config.CONFIG.isPath("jump-validation.enabled")
            && Config.CONFIG.getBoolean("jump-validation.enabled");
        MAX_JUMP_DISTANCE       = Config.CONFIG.isPath("jump-validation.max-distance")
            ? Config.CONFIG.getDouble("jump-validation.max-distance")      : 5.0;
        MAX_HORIZONTAL_DISTANCE = Config.CONFIG.isPath("jump-validation.max-horizontal")
            ? Config.CONFIG.getDouble("jump-validation.max-horizontal")    : 4.0;
        MAX_VERTICAL_UP         = Config.CONFIG.isPath("jump-validation.max-vertical-up")
            ? Config.CONFIG.getDouble("jump-validation.max-vertical-up")   : 1.0;
        MAX_VERTICAL_DOWN       = Config.CONFIG.isPath("jump-validation.max-vertical-down")
            ? Config.CONFIG.getDouble("jump-validation.max-vertical-down") : 3.0;
    }

    private static void initJumpTypes() {
        JUMP_TYPES_ENABLED = Config.CONFIG.isPath("jump-types.enabled")
            && Config.CONFIG.getBoolean("jump-types.enabled");
        ...
        if (Config.CONFIG.isPath("jump-types.types")) {
            for (String type : Config.CONFIG.getChildren("jump-types.types")) {
                ...
                JUMP_TYPE_ENABLED.put(type, Config.CONFIG.isPath(p + ".enabled")
                    && Config.CONFIG.getBoolean(p + ".enabled"));
                JUMP_TYPE_CHANCE.put(type, Config.CONFIG.isPath(p + ".chance")
                    ? Config.CONFIG.getDouble(p + ".chance") : 0.0);
            }
        }
    }

    private static void initMemory() {
        BLOCK_CLEANUP_DISTANCE = Config.CONFIG.isPath("memory.block-cleanup-distance")
            ? Config.CONFIG.getInt("memory.block-cleanup-distance") : 100;
        CLEANUP_INTERVAL       = Config.CONFIG.isPath("memory.cleanup-interval")
            ? Config.CONFIG.getInt("memory.cleanup-interval")       : 100;
        TRAIL_KEEP_BEHIND      = Config.CONFIG.isPath("memory.trail-keep-behind")
            ? Config.CONFIG.getInt("memory.trail-keep-behind")      : 10;
    }

    private static void initGhostMode() {
        GHOST_MODE_ENABLED = Config.CONFIG.isPath("ghost-mode.enabled")
            && Config.CONFIG.getBoolean("ghost-mode.enabled");
        GHOST_SHOW_TOP     = Config.CONFIG.isPath("ghost-mode.show-top")
            ? Config.CONFIG.getInt("ghost-mode.show-top")     : 3;
        GHOST_TRANSPARENCY = Config.CONFIG.isPath("ghost-mode.transparency")
            ? Config.CONFIG.getDouble("ghost-mode.transparency") : 0.5;
    }

    private static void initGenerationProbabilities() {
        ...
        if (MIN_Y >= MAX_Y) {
            ...
            LoParkour.getPlugin().getLogger().severe(
                "min-y >= max-y in generation.yml — using defaults 100/200.");
        }
    }
}
